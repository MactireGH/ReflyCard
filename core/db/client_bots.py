...
def create_client_bot(user_id: int, bot_name: str, bot_username: str, bot_token: str, bot_created: str) -> ClientBot:
    """Creates and saves new client bot to the database."""

    with closing(get_connection()) as connection, closing(connection.cursor()) as cursor:
        connection, cursor = cast(sqlite3.Connection, connection), cast(sqlite3.Cursor, cursor)

        cursor.execute("""INSERT INTO client_bots (user_id, bot_name, bot_username, bot_token, bot_created)
                          VALUES (?, ?, ?, ?, ?)""", (user_id, bot_name, bot_username, bot_token, bot_created))
